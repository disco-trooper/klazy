 ...
 */
async function selectService(services, searchTerm, allNamespaces, question = 'Select service:') {
    ...
    if (searchTerm) {
        const filtered = (0, fuzzy_1.fuzzyFilter)(serviceNames, searchTerm);
        if (filtered.length === 0) {
    ...
    const selected = await (0, cli_1.select)({ question, options: serviceNames, autocomplete: true });
    if (!selected)
        return undefined;
