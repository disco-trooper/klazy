...
async function copyFiles(srcArg, destArg, allNamespaces = false) {
    ...
    if (src.includes(':')) {
        ...
        if (!isPathSafe(dest)) {
            console.log((0, colors_1.colorize)('Error: Destination path outside allowed directories', 'red'));
            return;
        }
        ...
        if (!isPathSafe(src)) {
            console.log((0, colors_1.colorize)('Error: Source path outside allowed directories', 'red'));
            return;
        }
