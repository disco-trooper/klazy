...
async function copyFiles(srcArg, destArg, allNamespaces = false) {
  ...
  if (!srcArg) {
    ...
    const idx = await select(displayNames, 'Select pod:');
    if (idx === -1) return;
    const selectedPod = pods[idx];
    const ns = selectedPod.namespace || getCurrentNamespace();

    const direction = await select(['From pod to local', 'From local to pod'], 'Copy direction:');
    if (direction === -1) return;

    const remotePath = await input('Remote path (in pod): ');
    const localPath = await input('Local path: ');

    if (direction === 0) {
      execSync(`kubectl cp ${ns}/${selectedPod.name}:${remotePath} ${localPath}`, { stdio: 'inherit' });
    } else {
      execSync(`kubectl cp ${localPath} ${ns}/${selectedPod.name}:${remotePath}`, { stdio: 'inherit' });
    }
  ...
  if (src.includes(':')) {
    ...
    execSync(`kubectl cp ${ns}/${selectedPod.name}:${pathPart} ${dest}`, { stdio: 'inherit' });
    console.log(colorize('Copy completed', 'green'));
    ...
    execSync(`kubectl cp ${src} ${ns}/${selectedPod.name}:${pathPart}`, { stdio: 'inherit' });
    console.log(colorize('Copy completed', 'green'));
  } else {
