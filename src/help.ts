...
const optionMap: Record<string, OptionDescription> = {
    ...
    'logs': ['stream logs from a pod (follows by default)', 'use --no-follow for one-shot output', 'use --pipe <cmd> to pipe through external tool'],
    'logs -p': 'logs with context/ns picker (laku style)',
    'logs --pipe tspin': 'pipe logs through tailspin (or any tool)',
